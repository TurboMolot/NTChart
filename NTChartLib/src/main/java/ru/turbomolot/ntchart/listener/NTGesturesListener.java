...
public class NTGesturesListener implements View.OnTouchListener {
    ...
    public NTGesturesListener(NTChart chart) {
        if (chart == null)
    ...
    protected boolean applyMatrix(PointF startPoint, Matrix matrix) {
        ...
        if (chart != null) {
            List<ISeries<? extends IPoint>> series = chart.getSeries();
            if(series != null && !series.isEmpty()) {
                for (ISeries<? extends IPoint> it : series) {
                    RectF pos = it.getRenderPosition();
                    if(pos.contains(startPoint.x, startPoint.y) && it.getPointCount() > 0) {
                        matrix.getValues(matrixValues);
                        float scaleX = matrixValues[Matrix.MSCALE_X];
                        float scaleY = matrixValues[Matrix.MSCALE_Y];

                        float offX = matrixValues[Matrix.MTRANS_X];
                        float offY = matrixValues[Matrix.MTRANS_Y];
